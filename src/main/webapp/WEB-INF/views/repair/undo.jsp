...
<div class="list">
    ...
    <div class="list-bd">
        ...
        <c:choose>
            ...
            <c:otherwise>
                <a href="/dormitory/repair/undo?page=${pageInfo.pageNum-1}" target="_self">上一页</a>
            </c:otherwise>
        </c:choose>
        <!-- 此处引入当前页面信息 -->
        ...
        <c:choose>
            ...
            <c:otherwise>
                <a href="/dormitory/repair/undo?page=${pageInfo.pageNum+1}" target="_self">下一页</a>
            </c:otherwise>
        </c:choose>向第 <input id="inputpage" type="text"> 页
        <a class="skip" onclick="jump()">跳转</a>
        <script type="text/javascript">
            function jump() {
                window.location.href = "/dormitory/repair/undo?page=" + $("#inputpage").val();
            }
        </script>
    </div>
</div>
